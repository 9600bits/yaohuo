...
当前 1.7 版本在二改版基础上继续适配 `yaohuo.me`，重点提升回复稳定性、新回复提醒、帖子/列表缓存和请求可靠性。
...
- 修复回复帖子提示「页面已过期」（自动携带并刷新官方 CSRF 令牌）。
- 修复评论输入框弹出输入法时跳动的问题。
- 新增「别人回复我」提醒：60 秒检测一次新回复，弹系统通知，点击直达帖子详情。
- 新增帖子列表、帖子详情本地缓存，重复进入秒开。
- 统一网络请求层：自动携带 Cookie、15 秒超时、失败统一提示。
- 首页消息角标节流，减少重复请求；首页 Logo 本地化。
- 精简敏感权限；版本号升级至 1.7。
...
- 板块列表新增“全部 / 精选 / 新帖”切换，找新帖和精华帖更方便。
- 首页新增“活动线报 / 风云榜 / 全站热帖”入口。
- 新增原生风云榜页面，支持查看帖子、回复、妖晶、经验、人气排行。
- 我的收藏页面新增删除收藏按钮，可直接在 App 内管理收藏。
- 帖子内容新增代码块识别，代码显示更清楚，并支持一键复制。
- 新增原生修改帖子页面，自己的帖子可以在 App 内直接编辑标题和内容。
- 修复修改帖子按钮不显示的问题。
- 修复帖子浏览次数显示不准确的问题。
- 修复全站热帖、搜索和部分列表只能加载第一页或加载数量不完整的问题。
- 优化帖子图片、链接、代码内容混排时的显示效果。
